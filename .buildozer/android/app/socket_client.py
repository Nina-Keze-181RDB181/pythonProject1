...
HEADER_LENGTH = 10
client_socket = None
client_list = {'pass1':'client1','pass2':'client2'}
# Connects to the server
def connect(ip, port, my_username, password, error_callback):
    ...
    try:
        if client_list[username] == password:
            message = "Autorizācija veiksmīga"
            client_socket.send(message.encode('utf-8'))
        else:
            message = "Autorizācija nav veiksmīga"
            client_socket.send(message.encode('utf-8'))
    except KeyError:
        message = "Jauns lietotājs!"
        client_socket.send(message.encode('utf-8'))
